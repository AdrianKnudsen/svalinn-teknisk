...
(function(){
  ...
  $$('.rv:not(.in)').forEach((el,i)=>{
    ...
    if(!reduce && !el.classList.contains('sl') && !el.classList.contains('sr') && !el.classList.contains('sb') && !el.classList.contains('st')){
  ...
  $$('a[href^="#"]').forEach(a=>{
    ...
    a.addEventListener('click',(e)=>{ e.preventDefault(); scrollToSection(id); });
  });
  ...
  /* ============================================================
     ...
     ============================================================ */
  (function(){
    const fill=$('#socFill'), st=$('#socState'), pc=$('#socPct'), pw=$('#socPower');
    if(!fill) return;
    const bottomY=226, fullH=204, topMin=22;
    let t=0;
    function frame(){
      t+=0.006;
      // soc oscillates 0.35..0.95 over a slow cycle
      const soc=0.65+0.30*Math.sin(t);
      ...
      if(rate>0.12){ st.textContent='LADER'; st.setAttribute('fill','#47D7AC'); pw.textContent='+'+Math.round(40+rate*40)+' kW'; pw.setAttribute('fill','#47D7AC'); }
      else if(rate<-0.12){ st.textContent='TØMMER'; st.setAttribute('fill','#59CBE8'); pw.textContent='−'+Math.round(30+(-rate)*35)+' kW'; pw.setAttribute('fill','#9fd8ee'); }
      else { st.textContent='HOLDER'; st.setAttribute('fill','#cfe0ee'); pw.textContent='≈ 0 kW'; pw.setAttribute('fill','#cfe0ee'); }
      if(!reduce && document.body.dataset.motion!=='off') raf=requestAnimationFrame(frame);
    }
    let raf;
    ...
    if(reduce){ // static mid state
      const h=0.7*fullH; fill.setAttribute('y',bottomY-h); fill.setAttribute('height',h); pc.textContent='70%'; st.textContent='LADER';
    }
  })();
  ...
  /* ============================================================
     ...
     ============================================================ */
  (function(){
    const chart=$('#simChart'); if(!chart) return;
    const HRS=24;
    // ---- authored daily profiles ----
    const price =[.42,.40,.40,.41,.45,.55,.85,1.20,1.35,1.10,.75,.55,.48,.45,.50,.62,.88,1.25,1.50,1.40,1.05,.78,.60,.48];
    const solar =[0,0,0,0,0,2,12,30,55,80,100,115,120,118,105,82,55,28,10,2,0,0,0,0];
    const base=40;
    const demand=[30,28,28,30,32,36,42,46,48,50,52,54,55,54,52,50,48,46,44,42,38,35,33,31];
    // Svalinn schedule
    const chillerS=[40,40,40,38,38,36,18,10,20,45,78,86,90,86,76,54,26,10,8,16,34,38,40,38];
    const socS    =[49,53,58,64,67,70,70,59,44,32,30,41,55,70,83,93,95,86,70,55,44,42,44,47];
    // Naive (no storage, follows demand)
    const chillerN=[40,38,38,40,42,46,52,56,58,60,62,64,65,64,62,60,58,56,54,52,48,45,43,41];
    const socN = new Array(24).fill(50);
    const gridOf=(ch)=>ch.map((c,i)=>Math.max(0, c+base-solar[i]));
    const gridS=gridOf(chillerS), gridN=gridOf(chillerN);

    // phases & decisions (Svalinn)
    function phase(h){
      if(h<6) return {ph:'NATT', d:'Billig nattstrøm — lader termisk lager i forkant av morgentopp.'};
      if(h<10) return {ph:'MORGENRUSH', d:'Dyreste timer — henter kulde fra lager, kjøper minimalt fra nett.'};
      if(h<16) return {ph:'SOL', d:'Soloverskudd dekker drift og lader lageret. Tilnærmet null nettkjøp.'};
      if(h<21) return {ph:'KVELDSTOPP', d:'Ny pristopp — tømmer lager fremfor å kjøpe dyr effekt.'};
      return {ph:'NATT', d:'Pris faller — starter forsiktig opplading mot neste døgn.'};
    }
    function phaseN(h){
      if(h>=6&&h<10) return 'Kjøper full effekt i den dyreste morgentimen.';
      if(h>=16&&h<21) return 'Kjøper full effekt i kveldstoppen — ingen lastutjevning.';
      if(h>=10&&h<16) return 'Bruker noe sol, men overskudd selges billig ut.';
      return 'Konstant drift uavhengig av pris.';
    }

    // ---- chart geometry ----
    const W=1000,H=320, padR=8;
    ...
    function smooth(points){ // catmull-rom -> path
    ...
    let mode='svalinn', curHour=13, playing=false, timer=null;
    ...
    function valAt(arr,h){ const i=Math.floor(h), f=h-i; return lerp(arr[i], arr[Math.min(i+1,23)], f); }

    function updateScrub(){
      const s=seriesFor();
      const x=X(curHour);
      const line=$('#scrubLine'); if(line){ line.setAttribute('x1',x); line.setAttribute('x2',x); }
      const dp=$('#dotPrice'), dc=$('#dotChiller'), dg=$('#dotGrid');
      if(dp){ dp.setAttribute('cx',x); dp.setAttribute('cy',yPrice(valAt(price,curHour))); }
      if(dc){ dc.setAttribute('cx',x); dc.setAttribute('cy',yKW(valAt(s.chiller,curHour))); }
      if(dg){ dg.setAttribute('cx',x); dg.setAttribute('cy',yKW(valAt(s.grid,curHour))); }
    }

    // ---- readouts ----
    const elClock=$('#simClock'), elPhase=$('#simPhase'), elPhaseD=$('#simPhaseDesc');
    const roDec=$('#roDecision'), roPrice=$('#roPrice'), roGrid=$('#roGrid'), roSoc=$('#roSoc');
    const fillBar=$('#simFill'), marker=$('#simMarker');

    function fmtHour(h){ const hh=Math.floor(h)%24; const mm=Math.round((h-Math.floor(h))*60); return String(hh).padStart(2,'0')+':'+String(mm).padStart(2,'0'); }

    function render(){
      const s=seriesFor();
      const pr=valAt(price,curHour), gr=valAt(s.grid,curHour), so=valAt(s.soc,curHour);
      elClock.textContent=fmtHour(curHour);
      const hi=Math.round(curHour)%24;
      if(mode==='svalinn'){
        const ph=phase(hi); elPhase.textContent=ph.ph; elPhaseD.textContent=ph.ph==='SOL'?'Overskudd dekker drift — lader lager':(ph.ph==='MORGENRUSH'||ph.ph==='KVELDSTOPP'?'Dyr strøm — tømmer lager':'Lav pris — lader');
        roDec.textContent=ph.d;
      } else {
        elPhase.textContent= (pr>1.0?'DYR TIME':(solar[hi]>40?'SOL':'DRIFT'));
        elPhaseD.textContent='Ingen lastutjevning';
        roDec.textContent=phaseN(hi);
      }
      roPrice.innerHTML=pr.toFixed(2).replace('.',',')+'<span class="u">kr/kWh</span>';
      ...
      fillBar.style.width=f+'%'; marker.style.left=f+'%';
      updateScrub();
    }

    function setHour(h,smoothMove){ curHour=clamp(h,0,23.99); render(); }

    // ---- mode toggle ----
    $$('#simMode button').forEach(b=>b.addEventListener('click',()=>{
      ...
      $$('#simMode button').forEach(x=>{ if(x.dataset.mode!=='naive') x.classList.remove('naive'); });
      drawStatic(); render();
    }));
    ...
    let dragging=false;
    track.addEventListener('pointerdown',(e)=>{ dragging=true; stop(); fromEvent(e); track.setPointerCapture(e.pointerId); });
    track.addEventListener('pointermove',(e)=>{ if(dragging) fromEvent(e); });
    track.addEventListener('pointerup',()=>{ dragging=false; });
    track.addEventListener('pointercancel',()=>{ dragging=false; });

    // ---- play ----
    const playBtn=$('#simPlay'), hint=$('#simHint');
    function setPlayIcon(p){ playBtn.innerHTML='<svg viewBox="0 0 24 24"><use href="#'+(p?'ic-pause':'ic-play')+'"/></svg>'; }
    let rafId=null, lastTs=0;
    const DAY_MS = reduce?40000:29000;   // varighet for et helt døgn (jevn fart)
    function stop(){ playing=false; if(rafId) cancelAnimationFrame(rafId); rafId=null; setPlayIcon(false); }
    function play(){
      playing=true; setPlayIcon(true); hint.textContent='Spiller av døgnet…';
    ...
    playBtn.addEventListener('click',()=>{ playing?stop():play(); if(!playing) hint.textContent='Dra i tidslinjen for å utforske.'; });

    // init
    drawStatic(); setHour(13);
  ...
  window.__svalinnApply=function(t){
    ...
    if(t.accent){
      ...
      // derive soft + line
      document.documentElement.style.setProperty('--accent-soft', hexA(t.accent,.14));
      document.documentElement.style.setProperty('--accent-line', hexA(t.accent,.34));
    }
  };
  function hexA(hex,a){
  ...
  (function(){
    ...
    secs.forEach(s=>{ if(s) s.classList.add('journey'); });
    ...
    function animateScrollTo(target, duration){
      ...
      if(reduce || Math.abs(dist)<2){ window.scrollTo(0,target); return; }
      // turn off CSS smooth + snap so they don't fight the rAF animation (jank)
      const prevSnap=root.style.scrollSnapType, prevBeh=root.style.scrollBehavior;
      ...
      (function step(now){
        ...
        else { root.style.scrollSnapType=prevSnap; root.style.scrollBehavior=prevBeh; }
      })(t0);
    }
    items.forEach((a,i)=>{
      a.addEventListener('click',(e)=>{
        ...
        const vh=innerHeight, h=s.offsetHeight;
        const target = (h<=vh) ? s.offsetTop-(vh-h)/2 : s.offsetTop-TOP_OFF;
    ...
    function update(){
      ...
      const mid=y+vh*0.42;
      let active=0;
      secs.forEach((s,i)=>{ if(s && s.offsetTop<=mid) active=i; });
      items.forEach((a,i)=>{
        a.classList.toggle('active', i===active);
        a.classList.toggle('done', i<active);
      });
      if(active!==lastActive){
        secs.forEach((s,i)=>{ if(s) s.classList.toggle('in-focus', i===active); });
        lastActive=active;
      }
      // drive top nav from same logic — nothing active while in hero (so Problemet ≠ hero)
      links.forEach(l=>l.classList.remove('active'));
      if(show){ const id=items[active].dataset.sec; if(map[id]) map[id].classList.add('active'); }
      if(fill){ const frac=items.length>1 ? active/(items.length-1) : 0; fill.style.height=(frac*100)+'%'; }
    }
